...
notebook.add(tab_config, text="설정")
...
btn_specs = [
    ("찾기", lambda: browse(_dic_dir,  "dir",  "DICOM Folder")),   # 0
    ("찾기", lambda: browse(_out_dir,  "dir",  "Output Folder")),   # 1
    (None,   None),                                             # 2
    (None,   None),                                             # 3
    ("찾기", lambda: browse(_dict_file,"file", "Tag Dictionary")),   # 4
    ("찾기", lambda: browse(_dciodvfy,"file", "dciodvfy.exe 선택")), # 5 ★
    ("저장", lambda: save_as(_manifest_path,"Manifest File 저장",".json")),# 6
    ("저장", lambda: save_as(_excel_path,  "Tags Excel 저장",  ".xlsx"))   # 7
]
dump_cb = ttk.Checkbutton(
    root,
    text="dciodvfy -dump 출력까지 저장",
    variable=_dump_flag,
...
# ── Logbox     (row 10)
logbox = scrolledtext.ScrolledText(root, height=15, width=92, state=DISABLED)
...
notebook.add(ko_tab, text="설명 (KOR)")
...
def convert_validate():
    ...
    run_btn = ttk.Button(tab_config,  text="실행", command=lambda: threading.Thread(target=convert_validate, daemon=True).start())
    ...
    try:
        ...
        log(f"[info] 사전 태그 {len(dmap)}개 로드" if dmap else "[info] 사전 미사용")
